...
class ImgProcess:
    ...
    核心影像處理模組職責已徹底解耦, 分別提供獨立的 EMBL 擷取與 Video 解碼功能
    """
...
class ImgCapture(threading.Thread):
    ...
    繼承自 threading.Thread 的獨立執行緒負責透過 Windows 原生 Win32 API (Overlapped I/O)
...
class apiStreamMgr:
    """
    Stream Manager API
    USB 影像串流的總管類別負責統一管理資料擷取執行緒, 影像處理引擎與佇列 (Queue) 的生命週期, 
