        ...
        try:
            if self._usb_mgr:
                ...
                if val == self.OV195_ID:
                    return True
                raise RuntimeError(f"Connection OK but Value mismatch. Expected {hex(self.OV195_ID)}, got {hex(val)}")
            raise RuntimeError("without USBMGR")
        except Exception as e:
            self._log(f"Error: _connEG195_update(): {e}")
            return False
...
# Interface ---------------------------------------------------------------------------------------------
    def loadMainFirmware(self):
        ...
        if self._venus_api._isUsbInited:
            ...
            try:
                ...
                # path = r'D:\GitHub\blockly_html\OAX4000_general_eagle_v0.bin'
    ...
    def abort_test(self, test_mode=0):
        ...
        if test_mode == 1:
            ...
            if script_sts == 0x81:
                self._usbI2cSingleWrite(self.chip_id_script, 0x1201, 0xF, self.bit)
                return 0
        return -1
