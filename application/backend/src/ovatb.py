...
# Internal ---------------------------------------------------------------------------------------------
    def __init__(self, parent):
        ...
        self._connEG195 = self._connEG195_update()
        self._resultBurstLength = 128
        self._resultSize = 0x2400
        self._resultArray = (ctypes.c_uint8 * self._resultSize)(*([0] * self._resultSize))
        self._memory_map_addr = {'default':0x80200000, 'fcpSta':0x801C2000, 'testcases':0x801C8000, 'Embbed':0x801cB000, 'setting':0x801B1000,
                                    'FcpStat':0x801C2000, 'cmdSta':0x801D3000, 'testLog':0x801D6000, 'testLog2':0x801D6000}
    ...
    def _log(self, message):
        ...
        print(f"[{ts}][{'OVATB':>16}] {message}")

    def _connEG195_update(self):
        data = (ctypes.c_ubyte * 1)()
        try:
            if self._usb_mgr:
                ret = self._usb_mgr.drv_sccb_read(self.OV195_ID, 0x1001, 1, data, self.OV195_WIDTH)
                val = data[0]
                if ret != 1:
                    raise RuntimeError("Scan failed with OV195")
                if val == self.OV195_ID:
                    return True
                else:
                    self._log(f"Warning: Connection OK but Value mismatch. Expected {hex(self.OV195_ID)}, got {hex(val)}")
                    return False
                    # return True
            else:
                raise RuntimeError("without USBMGR")
    ...
    def _usbI2cMultiRead(self, Devid, Addr, Length, width):
        ...
        if not self._venus_api:
            self._log("Venus API not inited")
            return None
    ...
    def _usbI2cMultiWrite(self, DevId, Addr, Length, value, width):
        ...
        if DevId in (0x00, 0xFF):
            ...
            return 0
        if self._isBusy:
            self._log("_usbI2cMultiWrite() is busy")
            return 0
        self._isBusy = True
        try:
            # 處理 ctypes 資料結構
    ...
    def _sccb_write_table(self, table_list):
        for table in table_list:
            ...
            if w_size < 256:
                ...
                self._usbI2cMultiWrite(self.chip_id, table[0], w_size, write_data, self.bit)
            else:
                self._log(f'_sccb_write_table length error: {w_size} > 255')
    ...
    def _burst_mem(self, start_addr, data):
        ...
        while current_index < data_length:
            ...
            try:
                self._usbI2cMultiWrite(self.chip_id, addr, write_length, write_data, self.bit)
            except Exception as e:
                self._log(f"_burst_mem(), _usbI2cMultiWrite({self.chip_id:02X}, {addr:04X}, {write_length}, {self.bit:02X}): {e}")
            time.sleep(0.001)
    ...
    def _mem_map(self, map_addr):
        ...
        self._usbI2cMultiWrite(self.chip_id, 0x9644, 4, write_data, self.bit)
    ...
    def _updateMemoryAddrMapping(self, item):
        ...
        try:
            ...
            self._log(f"_updateMemoryAddrMapping(),{e}")
...
# Interface ---------------------------------------------------------------------------------------------
    def loadMainFirmware(self):
        ...
        if self._venus_api._isUsbInited:
            ...
            try:
                magic = self._usbI2cMultiRead(self.chip_id, 0xB8DC, 4, self.bit)
                if magic == 0x5454564F:
                ...
                for _ in range(500):
                    time.sleep(0.01)
                    data = self._usbI2cSingleRead(self.chip_id, 0xB8DC, self.bit)
    ...
    def is_test_running(self):
        state = self._usbI2cSingleRead(self.chip_id, 0x3000, self.bit)
    ...
    def activate_test(self):
        ...
        self._usbI2cSingleWrite(self.chip_id, 0x3000, 0, self.bit)
        return 0

    def abort_test(self):
        """Activate Test Board to stop test"""
        self._usbI2cSingleWrite(self.chip_id, 0x3001, 1, self.bit)
        return 0
    ...
    def is_device_atb(self):
        ...
        try:
            ...
            data = self._usbI2cSingleRead(self.chip_id, 0x80FD, self.bit) or 0
            ...
            self._log(f"is_device_atb() Error: {e}")
            return False
        ...
        try:
            ...
            for read_index in range(total_reads):
                ...
                if read_index < skip_index_threshold or read_index == total_reads - 1:
                    # 這裡回傳的是 List[int]
                    read_data_list = self._usbI2cMultiRead(self.chip_id, address, self._resultBurstLength, 0x10)
            ...
            self._log(f"readTestResult Error: {e}")
    ...
    def set_sccbs_slave(self):
        ...
        """
        self._usbI2cSingleWrite(self.chip_id, self.OAX4K_SCCBS_CFG_ADDRESS, 0x03, self.bit)

    def set_sccbs_bypass(self):
        """Set OAX4k SCCBS to be bypass mode
        """
        self._usbI2cSingleWrite(self.chip_id, self.OAX4K_SCCBS_CFG_ADDRESS, 0x10, self.bit)
    ...
    def init_tcal9539(self):
        try:
            if self._checkTcal9539Exist():
                ...
                self._usbI2cSingleWrite(self.chip_id, self.OAX4K_FW_TCAL9539OUT_B0_ADDR, 0x00, self.bit)  # Byte value mapping to TI-TCAL9539 Register 0x02
                self._usbI2cSingleWrite(self.chip_id, self.OAX4K_FW_TCAL9539OUT_B1_ADDR, 0x17, self.bit)  # Byte value mapping to TI-TCAL9539 Register 0x03
                self._usbI2cSingleWrite(self.chip_id, self.OAX4K_FW_TCAL9539DIR_B0_ADDR, 0x00, self.bit)  # Byte value mapping to TI-TCAL9539 Register 0x06
                self._usbI2cSingleWrite(self.chip_id, self.OAX4K_FW_TCAL9539DIR_B1_ADDR, 0x00, self.bit)  # Byte value mapping to TI-TCAL9539 Register 0x07
                self.set_sccbs_slave()
                return 0
            else:
    ...
    def set_sensor_error_bypass(self, val):
        self._usbI2cSingleWrite(self.chip_id, self.OAX4K_FW_ENFLAG_ADDR, val, self.bit)

    def set_i2c_route(self, val):
        """Update I2C configurated value to ATB"""
        try:
            if not self._oax4k_only:
                data = self._usbI2cSingleRead(self.chip_id, self.OAX4K_FW_TCAL9539OUT_B0_ADDR, self.bit)
                if data:
                    if val == 1:
                        data &= 0xFFFFFFF3
                    elif val == 0:
                ...
                self.set_sccbs_bypass()
                self._usbI2cSingleWrite(self.chip_id, self.OAX4K_FW_TCAL9539OUT_B0_ADDR, data, self.bit)
                self._usbI2cSingleWrite(self.TCAL9539_ID, 0x02, data, self.TCAL9539_WIDTH)
                self.set_sccbs_slave()
            return 0
        except Exception as e:
            self._log(f"set_i2c_route(): {e}")
            return -1
        finally:
            pass
    ...
    def set_strobe_route(self, val):
        ...
        try:
            ...
            self._log(f"set_strobe_route(): {e}")
            return -1
        finally:
            pass
    ...
    def set_sensor_dovdd(self, val):
        ...
        try:
            ...
            self._log(f"set_sensor_dovdd(): {e}")
            return -1
        finally:
            pass
