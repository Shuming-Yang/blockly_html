...
class _SccbDrv:
    ...
    def _log(self, message):
        ...
        print(f"[{ts}][{'OVUSB::SCCBDRV':>16}] {message}")
...
class OvApiDrv(_SccbDrv):
    ...
    def _log(self, message):
        ...
        # %f 會顯示 6 位數的微秒
        ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
        print(f"[{ts}][{'OVUSB::OVAPIDRV':>16}] {message}")

    def __init__(self, ignoreKey=True):
        if not OvApiDrv._inited:
            try:
                self.ovApiIf = _OVBaseIF(OVAPI_LIB_DIR)
                curW = os.getcwd()
                os.chdir(curW)
                if ignoreKey:
                ...
                self._log(f"Cannot load OVBaseIFX64.dll: {e}")
    ...
    def drv_device_init(self, onoff):
        ...
        if not OvApiDrv._inited:
            if onoff == 0:
                ret = self.ovApiIf.pyFinishDriver()
            elif onoff == 1:
                ret = self.ovApiIf.pyInitOVDriver(0, 0)
                # RGBRAW
