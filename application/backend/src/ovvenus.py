...
# Internal ---------------------------------------------------------------------------------------------
    def __init__(self):
        ...
        self._atb_mgr = apiOvAtb(self)  # if set EG195 Work Around to be default, it can be initialized on Venus Manager init, otherwise it can be initialized on demand by caller.
        self._header_set = ""
    ...
    def _prepend_titan_config(self):
        ...
        if os.path.exists(header_path):
            try:
                self._log(f"Using external configuration from: {header_path}")
