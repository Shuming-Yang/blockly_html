...
# Interface ---------------------------------------------------------------------------------------------
    ...
    def download_binary(self, test_mode: int = 0, board_id: int = 2) -> int:
        ...
        try:
            ...
            if venus_mgr is None:
                raise RuntimeError("Venus Manager is None")
            dev_mgr = venus_mgr._dev_mgr
            if dev_mgr is None:
            ...
            if test_mode == 0:
                ...
                if self._settings is not None:
                    ...
                    ret = dev_mgr.write_txtgroup(data=self._settings)
                # 最後將核心測試的 Binary 執行檔寫入硬體記憶體
                if self._binary is not None:
                    self._log("download test binary")
                    ret = dev_mgr.write_testcases(data=self._binary)
            elif test_mode == 1:
