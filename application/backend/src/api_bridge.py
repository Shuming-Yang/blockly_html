...
# Internal ---------------------------------------------------------------------------------------------
    ...
    def _execute_and_monitor_binary_test(self) -> None:
        ...
        if self._venus_mgr._atb_mgr.activate_test() != 0:
        ...
        while True:
            time.sleep(1)
            if not self._venus_mgr._atb_mgr.is_test_running():
                break
            if self._stop_test_process:
                self._log(f"Test Aborted! abort_test_ret:{self._venus_mgr._atb_mgr.abort_test()}")
                break
    ...
    def _binary_test_worker(self, workspace_data: Optional[dict[str, Any]] = None, target_id: int = 1) -> None:
        ...
        try:
            ...
            if self._cases_mgr.generate_binary(safe_workspace.get('binary_json', {}), self._project_dir) != 0:
                raise RuntimeError("Generate Binary Date Failed!")
            # 將編譯完成的 Binary 檔案下載至實體硬體板
            if self._cases_mgr.download_binary(self._test_mode, board_id) != 0:
    ...
    def _script_test_worker(self, scripts_datas: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            # 依序遍歷並傳送所有腳本檔案內容
            for item in scripts_datas:
                # 優先檢查是否收到中斷要求
                if self._stop_test_process:
                    self._log("Test Aborted before starting next file!!!")
                    break
                # 整理內容並直接初始化字典
                test_header = "--- [Header Settings] ---\n" + "\n".join(header) + "\n\n"
                test_content = f"--- {item.get('name', 'Main Script')} ---\n{item['content']}\n\n"
                script_ws_data = {"test_header": test_header, "test_content": test_content}
                self._test_index += 1
                self._project_dir = item.get('dir', "")
                self._workspace_image_base64 = item.get('image', None)
                content = str("\n".join(header) + "\n" + item['content'])
                
                name = item.get('name', 'Main Script')
                ws_id = item.get('id', '') # 獲取 UID
                self._log(f"Processing ({self._test_index}/{self._test_total}): {name}")
                
                # 通知前端：開始跑這個腳本
                if self._window:
                    self._window.evaluate_js(f"window.handleBackendEvent('SCRIPT_RUNNING', {{'id': '{ws_id}', 'name': '{name}'}})")

                if self._venus_mgr._mcu_mgr is not None:
                    self._venus_mgr._mcu_mgr.send_script_content(content)
                ...
                while True:
                    ...
                    if self._venus_mgr.read_script_status() == 0x1:
                        ...
                        # 通知前端：腳本跑完了
                        if self._window:
                            self._window.evaluate_js(f"window.handleBackendEvent('SCRIPT_FINISHED', {{'id': '{ws_id}', 'name': '{name}', 'status': 'success'}})")
                        break
                    # 若在執行途中收到中斷要求, 主動向硬體發送終止訊號
                    if self._stop_test_process:
                        self._log(f"Test Aborted! abort_test_ret:{self._venus_mgr._mcu_mgr.abort_test()}")
                        if self._window:
                            self._window.evaluate_js(f"window.handleBackendEvent('SCRIPT_FINISHED', {{'id': '{ws_id}', 'name': '{name}', 'status': 'error'}})")
                        break
    ...
    def _monitor_atb_config_process(self) -> None:
        ...
        while self._cfg_mgr._cfg_process and self._cfg_mgr._cfg_process.is_alive():
            time.sleep(0.5)
...
# Interface ---------------------------------------------------------------------------------------------
        ...
        try:
            ...
            if script_sts in (0x0, 0x80):
                ...
                if self._venus_mgr._atb_mgr.is_device_atb():
                    ...
                    self._venus_mgr._atb_mgr.loadMainFirmware()
                    if self._venus_mgr._atb_mgr.is_test_running():
                        ret = 1
                    else:
    ...
    def view_report(self, index: int = -1) -> None:
        """[API] 透過 Report Manager 開啟測試報告"""
        self._report_mgr.open_report(index)

    def js_log(self, message: str, tag: str = "") -> None:
        ...
        # 取得對應等級的圖標
        icon = level_icons.get(tag.upper(), "")
        # 固定標籤為 "JS Console", 訊息僅加上圖標
        global_log(tag="JS Console", message=f"{icon}{message}")
    ...
    def set_maximized_vc(self, channel: int) -> None:
        """前端通知目前放大的 VC 通道, -1 代表無放大以此控制背景 IQ 運算以節省 CPU"""
    ...
    def sign_project(self, project_data: dict[str, Any]) -> str:
        """
        [API] 簽署專案: 僅回傳 HMAC 簽章字串, 由前端負責回填
        """
        try:
            # 呼叫安全性模組計算簽章
            return ProjectValidator.generate_signature(project_data)
        except Exception as e:
            self._log(f"Error generating signature: {e}")
            return ""

    def verify_project(self, project_data: dict[str, Any]) -> dict[str, Any]:
        """
        [API] 驗證專案: 讀取檔案後檢查簽章是否與內容吻合
        調用語法: window.pywebview.api.verify_project(data_object)
        回傳格式: {"valid": Boolean, "reason": String}
        功能: 後端會使用相同的隱藏金鑰重算一次 HMAC, 若與檔案內容不符則回報錯誤
        """
        try:
            # 呼叫安全性模組比對 HMAC 簽章
            return ProjectValidator.verify(project_data)
        except Exception as e:
            self._log(f"Error verifying project: {e}")
