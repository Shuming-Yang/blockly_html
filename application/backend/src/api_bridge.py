...
# Internal ---------------------------------------------------------------------------------------------
    def __init__(self) -> None:
        ...
        # 初始化硬體板管理員與設定管理員參考 (延遲實例化以節省啟動資源)
        self._venus_mgr: Optional[apiOvVenus] = None
    ...
    def _check_test_managers(self) -> None:
        ...
        if self._venus_mgr is None or getattr(self._venus_mgr, '_dev_mgr', None) is None:
            raise RuntimeError("Board Managers are None")
        if self._cfg_mgr is None:
            assert self._venus_mgr is not None
    ...
    def _finalize_test_and_report(self, board_id: int, report_workspace_data: dict[str, Any], worker_name: str) -> None:
        ...
        if self._report_mgr and report_workspace_data:
            ret = self._report_mgr.generate_report(
                ...
                serial_logs=list(self._serial_logs_buffer),
                _workspace_image_base64=self._workspace_image_base64,
                app_logs_list=self._get_app_logs())
            if ret != 0:
                self._log("Error: Generate Report Failed.")
        # 清除硬體通訊標頭紀錄以避免污染下次測試
        if self._venus_mgr and hasattr(self._venus_mgr, '_header_set'):
    ...
    def _script_test_worker(self, scripts_datas: list[dict[str, Any]]) -> None:
        ...
        script_ws_data: dict[str, Any] = {"test_header": "", "test_content": ""}
        try:
            self._test_mode = 1
            ...
            # 寫入預先配置好的硬體初始化標頭設定
            if self._venus_mgr and hasattr(self._venus_mgr, '_header_set') and getattr(self._venus_mgr, '_header_set'):
                script_ws_data["test_header"] += "--- [Header Settings] ---\n" + "\n".join(getattr(self._venus_mgr, '_header_set')) + "\n\n"
            # 依序遍歷並傳送所有腳本檔案內容
            ...
            # 強制進入收尾與報告生成流程
            self._finalize_test_and_report(1, script_ws_data, "_script_test_worker")
    ...
    def _init_venus_safely(self, skip_reinit: bool = False) -> None:
        ...
        if self._venus_mgr is None:
            self._venus_mgr = apiOvVenus()
            self._log("Initialized apiOvVenus instance.")
...
# Interface ---------------------------------------------------------------------------------------------
    ...
    def open_atb_config_window(self) -> int:
        ...
        try:
            ...
            assert self._venus_mgr is not None  # 讓 mypy 安心
            # 初始化 ATB 子系統
    ...
    def script_test_sendfile(self, project_dir: str = "", workspaceImageBase64: Optional[str] = None, scripts_datas: Optional[list[dict[str, Any]]] = None, testIndex: int = 1, testtotal: int = 1) -> int:
        ...
        try:
            ...
            # 初始化狀態與綁定資料
            self.start_serial_logging()
            self._workspace_image_base64 = workspaceImageBase64
            self._venus_mgr.venus_mcu_board_init()
            # 注入 Titan 硬體專屬的通訊標頭設定
            header = self._venus_mgr.venus_cfg_titan()
            scripts_datas[0]['content'] = "\n".join(header) + "\n" + scripts_datas[0]['content']
            self._test_index = testIndex
            self._test_total = testtotal
