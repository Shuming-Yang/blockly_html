...
class BlocklyInstructionGenerator:
    ...
    def _handle_config_mipi_dt(self, block: Dict[str, Any]) -> Tuple[str, str]:
        ...
        vc = str(block.get("vc", ""))
        dt = str(block.get("dt", ""))
        instruction = f"FC {self.config.CMD_MAP['config_mipi']} 3{stream}000{vc}{dt}; mipi stream{stream} vc{vc} dt"
    ...
    def _handle_config_mipi(self, block: Dict[str, Any]) -> Tuple[str, str]:
        ...
        if hsize != 0:
            new_instructions.append(f"FC {self.config.CMD_MAP['config_mipi']} 1{stream}{hsize:06X}; mipi hsize")
        if hsize != 0:
            new_instructions.append(f"FC {self.config.CMD_MAP['config_mipi']} 2{stream}{vsize:06X}; mipi vsize")
        if max != 0:
            new_instructions.append(f"FC {self.config.CMD_MAP['config_mipi']} 4{stream}{max:06X}; mipi max")
        if min != 0:
            new_instructions.append(f"FC {self.config.CMD_MAP['config_mipi']} 5{stream}{min:06X}; mipi min")
