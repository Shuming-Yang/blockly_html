...
(function() {
  ...
  const MAGIC_48 = ["48", "00", "00", "00"];
  const MAGIC_AA = ["AA", "00", "00"];
  let procedureDict = {};
  ...
  function IRtobinjson_process(Data) {
    ...
    targetBlocks.forEach(block => {
      if (block.type === "procedures_defnoreturn") {
        Mode_hex = formatDec(group_num, 4);
        procedureDict[block.name] = Mode_hex;
        group_num += 1;
      }
    });
    ...
    targetBlocks.forEach(rootBlock => {
      ...
      if(rootBlock.type === 'startup') {
        ...
        Mode_hex = INT_TASK_MAP[rootBlock.name] || '0000';
      } else {
        Mode_hex = procedureDict[rootBlock.name] || '0000';
      }
      // 取得該流程內包含的子步驟數量
      ...
      if (rootBlock.inputs) {
        for (let key in rootBlock.inputs) {
          ...
          // console.debug("handle commd", inputContent.type);
    ...
    console.log("=== Binary JSON Generation End ===");
  ...
  const BLOCK_HANDLERS = {
    ...
    'controls_whileUntil_ex': (block) => {
      return cmd_controls_whileUntil_ex(block);
    },
    'loop_control': (block) => {
      return cmd_loop_control(block);
    },
    // 預設行為：只提取 Opcode，無參數
    'default': (block) => {
      const hex = CMD_OP_MAP[block.type];
      console.debug(`Checking default for ${block.type}:`, hex); // 除錯用
      return {
  ...
  function cmd_set_var(data){
    ...
    if(subBlock){
      ...
      if(subBlock.type === 'read_device') {
        ...
        out_string = [dev, "00", "00", "00", ...address.match(/.{1,2}/g), ...lenSel.match(/.{1,2}/g), ...MAGIC_AA, var_num];
      } else if(subBlock.type === 'get_var') {
        // 來源為取得變數
        cmdHex = "0104";
        const var_in = VarSetHex(subBlock.var, 2);
        out_string = [...MAGIC_48, ...MAGIC_AA, var_in, "00", "00", "00", "04", ...MAGIC_AA, var_num];
      } else if (subBlock.type === 'num_hex'){
        // 來源為純 16 進位數字
        const lenSel = "00000004"
        const value = formatHex(subBlock.num_hex, 8);
        out_string = [...MAGIC_48, ...MAGIC_AA, var_num, ...lenSel.match(/.{1,2}/g), ...value.match(/.{1,2}/g)];
        ...
        out_string = [ "01", "00", "00", opHex, ...MAGIC_AA, op1var_num, ...MAGIC_AA, op0var_num, ...MAGIC_AA,var_num ];
        cmdHex = "0801";
        ...
        out_string = [...MAGIC_48, ...value.match(/.{1,2}/g), ...lenSel.match(/.{1,2}/g), ...MAGIC_AA, var_num];
        ...
        const value = (parseInt(base, 16) + parseInt(subBlock.offset, 16))
                ...
                .padStart(8, '0')
                .toUpperCase();
        out_string = [...MAGIC_48, ...value.match(/.{1,2}/g), ...emb_size.match(/.{1,2}/g), ...MAGIC_AA, var_num];
      } else if (subBlock.type === 'random_int'){
        // 來源為純 10 進位數字
        cmdHex = "0700";
        const min = formatDec(subBlock.min, 4);
        const max = formatDec(subBlock.max, 4);
        out_string = [...min.match(/.{1,2}/g), ...max.match(/.{1,2}/g), ...MAGIC_AA, var_num];
      } else {
        // 未知來源防呆
        out_string = ["77", "00"];
      }
  ...
  function cmd_write_device(data){
    ...
    if(subBlock){
      ...
      if(subBlock.type === 'get_var') {
        ...
        out_string = [dev, "00", "00", fromVar, ...address.match(/.{1,2}/g), ...lenSel.match(/.{1,2}/g), ...MAGIC_AA, var_num];
      }
    }
  ...
  function cmd_controls_if_custom(data){
    ...
    if(subBlock_a && subBlock_b){
      ...
      out_string = [opHex, "00", "00", "00", ...MAGIC_AA, vara || "00", ...MAGIC_AA, varb || "00"];
    }
    if(subBlock_do){
  ...
  function cmd_print(data){
    ...
    if (data.val.startsWith("${")) {
      ...
      out_string = [...MAGIC_AA, formatDec(index, 2)];
      return {cmdHex: "0901", string: out_string};
    } else {
      const rawVal = data.val.replace(/\\n/g, '\n').replace(/\\r/g, '\r');
      out_string = rawVal.split('').map(char => 
  ...
  function cmd_abort(data){
    ...
    out_string = [...MAGIC_48, ...address.match(/.{1,2}/g), ...w_size.match(/.{1,2}/g), ...value.match(/.{1,2}/g)];
  ...
  function cmd_create_timer(data){
    if( data.type === 'create_timer_random')
    {
      const delay_min = data.min;
      const delay_max = data.max;
      // 將微秒轉為 8 字元的 Hex，並拆分成 Byte 陣列
      const out_min = formatDec(delay_min, 4);
      const out_max = formatDec(delay_max, 4);
      out_string = [...out_min.match(/.{1,2}/g), ...out_max.match(/.{1,2}/g), "00", "00", "00", "00"];
      return {cmdHex: "0401", string: out_string};
    } else {
      const delay_ms = data.time;
      // 將微秒轉為 8 字元的 Hex，並拆分成 Byte 陣列
  ...
  /**
   * 處理積木 (controls_whileUntil_ex)。
   * @param {Object} data - 積木資料
   * @returns {Object} {cmdHex, string} 解析結果
   */
  function cmd_controls_whileUntil_ex(data){
    const out = procedureDict[data.do.extraState.name];
    out_string = ["01", "FF", "FF", "FF", "00", "00", ...out.match(/.{1,2}/g)];
    return {cmdHex: "0803", string: out_string};
  }
   ...
   * @returns {Object} {cmdHex, string} 解析結果
   */
  function cmd_loop_control(data){
    return {cmdHex: "0804", string: []};
  }
