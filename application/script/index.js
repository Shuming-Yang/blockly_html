  ...
  field.showEditor_ = async function() {
    ...
    if (window.pywebview && window.pywebview.api) {
      ...
      if (result && result.path) {
        ...
        if (!window.projectDir && typeof updateProjectDir === 'function') {
          updateProjectDir(absPath);
        }
        ...
        if (projectDir) {
          ...
          // 1. 統一路徑格式 (Windows 相容性與大小寫不敏感處理)
          const normalizedAbs = absPath.replace(/\\/g, '/').toLowerCase();
          const normalizedProj = projectDir.replace(/\\/g, '/').toLowerCase();
          ...
          // 3. 計算相對路徑：截斷專案路徑前綴並移除開頭斜線
          let relPath = absPath.substring(projectDir.length);
...
const customBlocksJson = [
  ...
  {
    "type": "placeholder_config_mipi",
    "message0": "Config MIPI (MCU Only)",
    "colour": 40,
...
const baseToolbox = {
  ...
  "contents": [
    ...
    },
    {
      "kind": "category",
      "name": "Timing",
      "custom": "DYNAMIC_TIMING",
      "contents": [
        { "kind": "block", "type": "placeholder_INT" },
    ...
    },
    {
      ...
      "contents": [
        { "kind": "block", "type": "placeholder_config_mipi" }
      ]
    }
  ]
};
...
function create_customer_toolbox() {
  workspace.registerToolboxCategoryCallback('DYNAMIC_ADVANCE', function(workspace) {
    ...
    // 處理僅限 MCU 的積木
    if (boardId === '1') {
      const index = contents.findIndex(c => c.type === 'placeholder_config_mipi');
      if (index !== -1) {
      ...
      // const getMipiErrorBlock = contents.find(c => c.type === 'placeholder_config_mipi');
      // if (getMipiErrorBlock) getMipiErrorBlock.type = 'config_mipi';
    } else {
      // 如果不是 MCU，則過濾掉所有僅限 MCU 的佔位符
      const mcuOnlyPlaceholders = ['placeholder_config_mipi'];
  ...
  workspace.registerToolboxCategoryCallback('DYNAMIC_TIMING', function(workspace) {
    // Use 'custom' property instead of 'name' for stability
    const categoryDef = baseToolbox.contents.find(c => c.custom === 'DYNAMIC_TIMING');
    let contents = categoryDef ? JSON.parse(JSON.stringify(categoryDef.contents)) : [];
    const boardId = String(window.GLOBAL_BOARD_ID || '1'); // Default to MCU

    // 處理通用佔位符替換
    const embBlock = contents.find(c => c.type === 'placeholder_INT');
    if (embBlock) {
      embBlock.type = (boardId === '1') ? 'wait_until_timeout' : 'int_task';
    }
