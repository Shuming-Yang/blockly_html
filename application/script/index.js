...
Blockly.Extensions.register('file_picker_extension', function() {
  ...
  field.showEditor_ = async function() {
    ...
    if (window.pywebview && window.pywebview.api) {
      ...
      const result = await window.pywebview.api.load_test_case_dialog(options);

      if (result && result.error) {
          if (result.error === 'ERROR_NO_PROJECT_DIR') {
            alert('Please save the project first to set the working directory.');
          } else if (result.error === 'ERROR_OUTSIDE_DIR') {
            alert('Please select a file within the project directory.');
          }
          return;
      }
    ...
    input.onchange = (e) => {
      ...
      if (file) {
        field.setValue(file.name); // 更新欄位值
      }
    };
    input.click();
  };
...
const customBlocksJson = [
  ...
  {
    "type": "placeholder_get_mipi_error",
    "message0": "Get MIPI Error (MCU Only)",
    "colour": 40,
    "tooltip": "This block is only available for MCU boards.",
  },
  {
    "type": "get_mipi_error",
    "message0": "%1 MIPI error",
    "args0": [
      {
        "type": "field_dropdown",
        "name": "vc_sel",
    ...
    "output": "String",
    "colour": 40,
    "tooltip": "Get the last MIPI error status for a specific Virtual Channel."
  },
  {
    "type": "num_hex",
    "message0": "hex %1",
  ...
  {
    "type": "placeholder_mipi_monitor",
    "message0": "MIPI Monitor (MCU Only)",
    "colour": 250,
    "tooltip": "This block is only available for MCU boards.",
  },
  {
    "type": "mipi_monitor",
    "message0": "Start MIPI monitor, threshold %1",
    "args0": [
      {
        "type": "field_number",
        "name": "threshold",
    ...
    "message1": "SOF/EOF check: %1 VC0 %2 VC1 %3 VC2 %4 VC3",
    "args1": [
      { "type": "field_checkbox", "name": "sof_eof_vc0", "checked": false },
      { "type": "field_checkbox", "name": "sof_eof_vc1", "checked": false },
      { "type": "field_checkbox", "name": "sof_eof_vc2", "checked": false },
      { "type": "field_checkbox", "name": "sof_eof_vc3", "checked": false }
    ],
    "message2": "Packet size check: %1 VC0 %2 VC1 %3 VC2 %4 VC3",
    "args2": [
      { "type": "field_checkbox", "name": "size_err_vc0", "checked": false },
      { "type": "field_checkbox", "name": "size_err_vc1", "checked": false },
      { "type": "field_checkbox", "name": "size_err_vc2", "checked": false },
      { "type": "field_checkbox", "name": "size_err_vc3", "checked": false }
    ],
    "message3": "CRC check: %1",
    "args3": [
  ...
  {
    "type": "timer_active",
    "message0": "Active Timer Thread %1 After %2 (ms)",
    "args0": [
      { "type": "field_dropdown", "name": "timer_sel", "options": getProcedureOptions },
      { "type": "field_input", "name": "times", "text": "5" }
    ],
    "previousStatement": null,
    "nextStatement": null,
    "colour": 380,
    "tooltip": "Activate the selected timer thread after specified delay",
  },
  {
    "type": "timer_thread",
    "message0": "Timer Thread %1 %2 %3",
    "args0": [
      { "type": "field_label_serializable", "name": "timer_sel", "text": "Timer" },
      { "type": "input_value", "name": "timer_mode", "check": "timer_type" },
      { "type": "input_statement", "name": "statements" }
    ],
    "colour": 380,
    "tooltip": "Execute statements at the Timer",
  },
  {
    "type": "timer_repeat",
    "message0": "Repeat mode per %1 ms Run Thread",
    "args0": [
      { "type": "field_number", "name": "time", "text": "0000" }
    ],
    "output": "timer_type",
  ...
  {
    "type": "timer_repeat_random",
    "message0": "Repeat mode per %1 ~ %2 ms Run Thread",
    "args0": [
      { "type": "field_number", "name": "min", "text": "0000" },
      { "type": "field_number", "name": "max", "text": "0100" },
    ],
  ...
  {
    "type": "timer_once",
    "message0": "Do one time",
    "output": "timer_type",
    "colour": 380,
    "tooltip": "Declare a one-time timer",
  },
  {
    "type": "return_void",
    "message0": "Return",
  ...
  function traverse(block) {
    ...
    if (!Blockly.Blocks[block.type]) {
      ...
      if (def) {
        ...
        // Iterate over all argsN (args0, args1, ...)
        for (let i = 0; ; i++) {
...
function create_cutomer_Timer() {
  // 註冊動態類別回呼
  workspace.registerToolboxCategoryCallback('DYNAMIC_PROCEDURES', function(workspace) {
    const contents = [];
    
    // 1. 建立「建立計時器」按鈕
    contents.push({'kind': 'button', 'text': '＋ Create New Timer', 'callbackKey': 'CREATE_PROC_BTN' });

    // 2. 如果沒有任何已建立的 Timer，則只顯示按鈕
    if (myProceduresList.length === 0) {
      return contents;
    }

    // 3. 加入固定的 Timer 功能積木 (active, repeat 等)
    contents.push({'kind': 'block', 'type': 'timer_repeat',  'name': 'repeat' });
    contents.push({'kind': 'block', 'type': 'timer_repeat_random',  'name': 'repeat_random' });
    contents.push({'kind': 'block', 'type': 'timer_once',  'name': 'once' });
    contents.push({'kind': 'block', 'type': 'timer_active',  'name': 'active' });

    // 4. 根據已建立的 Timer 動態生成對應的 `timer_thread` 定義積木
    // 先取得工作區上已經存在的 timer_thread 積木，用於檢查是否重複
    const existingBlocks = workspace.getBlocksByType('timer_thread', false);
    const usedTimers = existingBlocks.map(b => b.getFieldValue('timer_sel'));

    myProceduresList.forEach(timerName => {
      const block = {
        'kind': 'block',
        'type': 'timer_thread',
        'name': 'thread',
      ...
      // 如果工作區已經有這個 Timer 的定義，則禁用工具箱裡的對應積木 (達成 Max 1 的效果)
      if (usedTimers.includes(timerName)) {
        block['disabled'] = true;
      }
  ...
  // 註冊「建立計時器」按鈕的點擊事件
  workspace.registerButtonCallback('CREATE_PROC_BTN', function(button) {
    const name = prompt("Timer Name：");
    if (!name) return; // User cancelled
    if (myProceduresList.includes(name)) {
      alert(`Timer with name "${name}" already exists.`);
      return;
    }

    // 1. 將名稱存入清單
    myProceduresList.push(name);

    // 2. 強制工具箱重新渲染，這會讓積木上的下拉選單重新抓取最新的 Timer 清單
...
function create_customer_device() {
  workspace.registerToolboxCategoryCallback('DYNAMIC_DEVICE', function(workspace) {
    ...
    // 處理僅限 MCU 的積木
    if (boardId === '1' && false) { // TOIMPLEMENT: Disable MIPI-related features for now.
      // 將佔位符替換為真實積木
      const mipiErrorBlock = contents.find(c => c.type === 'placeholder_mipi_monitor');
      if (mipiErrorBlock) mipiErrorBlock.type = 'mipi_monitor';

      const getMipiErrorBlock = contents.find(c => c.type === 'placeholder_get_mipi_error');
      if (getMipiErrorBlock) getMipiErrorBlock.type = 'get_mipi_error';
    } else {
      // 如果不是 MCU，則過濾掉所有僅限 MCU 的佔位符
      const mcuOnlyPlaceholders = ['placeholder_mipi_monitor', 'placeholder_get_mipi_error'];
      contents = contents.filter(block => !mcuOnlyPlaceholders.includes(block.type));
    }
...
function update_toolbox(board){
  // 預設為 1 (MCU Board), 2 為 ATB Board
  console.log("Target board:", board, "Type:", typeof board);

  const newToolbox = JSON.parse(JSON.stringify(baseToolbox));
  const boardId = String(board || window.GLOBAL_BOARD_ID || '1');

  if (boardId === '1') {
    // Filter by 'custom' property instead of 'name'
    newToolbox.contents = newToolbox.contents.filter(item => item.custom !== 'DYNAMIC_PROCEDURES');
  }
...
function flattenWorkspace() {
  ...
  class TempManager {
    ...
    get() {
      if (this.pool.length > 0) {
        ...
        console.log(`     [Temp] Reusing temp var: ${variableInfo.name}`);
        return variableInfo;
      }
      ...
      console.log(`     [Temp] Created new temp var: ${variableInfo.name} (ID: ${variableInfo.id})`);
      return variableInfo;
    }
    ...
    release(variableInfo) {
      if (variableInfo && variableInfo.name && variableInfo.name.startsWith('tmp')) {
        if (!this.pool.some(v => v.id === variableInfo.id)) {
          console.log(`     [Temp] Releasing temp var: ${variableInfo.name}`);
  ...
  function extractToFunction(input, bodyBlock) {
    ...
    console.log(`     Extracting body to function: ${funcName}`);
  ...
  function transformIfElif(ifBlock) {
    ...
    console.log(`  -> Transforming if-elif block: ${ifBlock.id} (found ${elifCount} elifs)`);
  ...
  function transformWhileLoop(whileBlock) {
      console.log(`  -> Transforming while loop: ${whileBlock.id}`);
  ...
  function processStatement(stmtBlock) {
    ...
    stmtBlock.inputList.forEach(input => {
      if (input.type === inputTypes.VALUE) {
        ...
        if (targetBlock && !isSimpleValue(targetBlock) && !isSetVarConstant && isTargetString && isParentAcceptString) {
          if (stmtBlock.type === 'set_var') {
             // [Fix] 如果是 set_var，則不提取頂層指令，只遞迴處理其子節點 (保留 a = cmd 結構)
             console.log(`[Flatten] Skipping top-level extraction for set_var ${stmtBlock.id}, processing children only.`);
             processExpressionChildren(targetBlock, stmtBlock);
          } else {
             console.log(`[Flatten] Found complex expression in ${stmtBlock.type} (ID: ${stmtBlock.id}), Input: ${input.name}`);
  ...
  function extractExpression(exprBlock, anchorStmt, parentInput) {
    console.log(`  -> Extracting ${exprBlock.type} (ID: ${exprBlock.id})`);
    ...
    try {
        ...
        console.log(`     Allocated temp var: ${tempVarInfo.name} (ID: ${tempVarInfo.id})`);
        ...
        console.log("     Connecting expression to set_var...");
        ...
        console.log("     Replacing original expression with get_var...");
        ...
        console.log("     Inserting set_var before anchor statement...");
        ...
        if (loopUpdateBlock) {
             console.log("     [Loop] Appending condition update to loop body...");
  ...
  // 從 Top Blocks 開始遍歷
  console.log("Processing Top Blocks...");
